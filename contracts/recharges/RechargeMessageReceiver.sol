...
contract RechargeMessageReceiver is RouterWrapper, OwnerWithdrawable {
	...
	modifier onlyMessageBus() {
		require(msg.sender == messageBus, 'RechargeMessageReceiver: caller is not message bus');
		_;
	}
	...
	function executeMessageWithTransfer(
		...
		require(executor == _executor, 'RechargeMessageReceiver: invalid executor');
		IFundPool fundPool = router.FundPool();
		token.safeApprove(address(fundPool), amount);
		try fundPool.celerExec(amount, message) {
		...
		token.safeApprove(address(fundPool), 0);
